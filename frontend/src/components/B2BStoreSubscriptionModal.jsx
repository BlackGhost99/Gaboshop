...
/**
 * Modal pour créer/modifier un abonnement store B2B
 */
const B2BStoreSubscriptionModal = ({ isOpen, onClose, subscription = null, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (subscription?.id) {
        res = await updateB2BStoreSubscription(subscription.id, formData);
      } else {
        res = await createB2BStoreSubscription(formData);
      }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={subscription ? 'Modifier l\'abonnement B2B' : 'Créer un abonnement B2B'}
      size="md"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Magasin B2B *</label>
          <select
          ...
          >
            <option value="">Sélectionner un magasin B2B</option>
            {stores.map(store => (
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Plan B2B</label>
          <select
            value={formData.plan_id}
            onChange={(e) => updateField('plan_id', e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
          >
            <option value="">Aucun plan (B2B Free)</option>
...
export default B2BStoreSubscriptionModal;
