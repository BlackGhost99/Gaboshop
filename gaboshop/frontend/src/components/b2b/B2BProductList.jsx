...
const B2BProductList = ({ products, onAddToCart, loading }) => {
	...
	const handleAddToCart = (product) => {
		const quantity = quantities[product.id] || product.b2b_pricings?.[0]?.min_quantity || 1;
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			{products.map((product) => {
				const b2bPrice = product.b2b_price || product.b2b_pricings?.[0]?.b2b_price;
				const minQuantity = product.b2b_pricings?.[0]?.min_quantity || 1;
					...
					>
						...
						<div className="p-4">
							...
							<button
								...
								disabled={!b2bPrice || product.stock < minQuantity}
